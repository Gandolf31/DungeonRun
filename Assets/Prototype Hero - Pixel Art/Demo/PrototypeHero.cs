...
public class PrototypeHero : MonoBehaviour {
    ...
    void Start ()
    {
        HSys = HealthBar.GetComponent<HealthSystem>();
        ScoreManager = GameObject.Find("ScoreManager");
        SCMgr = ScoreManager.GetComponent<ScoreMgr>();
    ...
    void Update ()
    {
        ...
        else if (Input.GetMouseButtonDown(0) && !m_dodging && !m_ledgeGrab && !m_ledgeClimb && !m_crouching && m_grounded && m_timeSinceAttack > 0.2f)
        {
            ...
            Collider2D[] collider2Ds = Physics2D.OverlapBoxAll(WPos.position, boxSize, 0);
            foreach (Collider2D collider in collider2Ds)
            {
                if(collider.tag == "Enemy")
                    collider.GetComponent<EnemyMgr>().Attacked();
            }
        }
    ...
    {
        if (other.gameObject.tag == "Enemy" && !m_dodging && tag == "Player" && !m_dead)
        {
            OnDamaged();
        }
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireCube(WPos.position, boxSize);
    }
}